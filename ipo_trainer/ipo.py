...
def compute_logprobs(model, input_ids, attention_mask, is_response_token, average_logps):
    #computes logps for a model, input_ids, attention_mask, and is_response_token
    ...
    if average_logps:
        return token_log_probs_masked.sum(dim=-1)/resp_mask.sum(dim=-1).float().clamp(min=1)
    else:
        return token_log_probs_masked.sum(dim=-1)

def compute_loss(logps_w, logps_l, logps_ref_w, logps_ref_l, beta):
    #computes loss and reward margin for IPO
...
def evaluate(model, reference_model, test_dataloader, device, beta, global_step, average_logps):
    ...
    with torch.inference_mode():
        for idx, batch in enumerate(test_dataloader):
            ...
            logps_w = compute_logprobs(model, input_ids_w, attention_mask_w, is_response_token_w, average_logps)
            logps_l = compute_logprobs(model, input_ids_l, attention_mask_l, is_response_token_l, average_logps)

            logps_ref_w = compute_logprobs(reference_model, input_ids_w, attention_mask_w, is_response_token_w, average_logps)
            logps_ref_l = compute_logprobs(reference_model, input_ids_l, attention_mask_l, is_response_token_l, average_logps)
...
def train(
    ...
    for e in tqdm.tqdm(range(num_epochs), desc="epoch"):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            #print progress every 500 batches or on first batch
            ...
            #compute logprobs for w and l chosen/rejectedresponses
            logps_w = compute_logprobs(model, input_ids_w, attention_mask_w, is_response_token_w, average_logps)
            logps_l = compute_logprobs(model, input_ids_l, attention_mask_l, is_response_token_l, average_logps)

            #compute logprobs for w and l chosen/rejectedresponses - no grad for reference model
            with torch.no_grad():
                logps_ref_w = compute_logprobs(reference_model, input_ids_w, attention_mask_w, is_response_token_w, average_logps)
                logps_ref_l = compute_logprobs(reference_model, input_ids_l, attention_mask_l, is_response_token_l, average_logps)
            ...
            if (idx + 1) % gradient_accumulation_steps == 0:
                ...
                wandb.log(
                    {
                        'ipo_train_loss_step': loss_sum/max(loss_count, 1), 
                ...
                ########## EVALUATION ##########
                # evaluate on test dataloader every batch
                # can change to evaluate every x gradient accumulation steps by changing the % 10 to % x
                # Currentlyvaluates every 10 gradient accumulation steps
                if (idx + 1) % (10 * gradient_accumulation_steps) == 0: #if (idx + 1) % 10 == 0:
                    evaluate(model, reference_model, test_dataloader, device, beta, global_step, average_logps)
                    model.train()

        # NOT USING PER EPOCH LOGGING BECAUSE WE ARE LOGGING PER STEP ABOVE (using 1 epoch only)
